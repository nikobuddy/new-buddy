...
const ManufacturerPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProductionInventory = async () => {
      const querySnapshot = await getDocs(
        collection(db, "productionInventory")
      );
      const items = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as ProductionItem[];
      setProductionInventory(items);
    };
  ...
  const handleUpdateItem = async (
    ...
    const itemRef = doc(db, "productionInventory", id);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <p className="mb-8 text-gray-600">
        Welcome, Manufacturer! Manage your production lines and track your
        shipments here.
      </p>
